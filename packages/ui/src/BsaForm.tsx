/**
 * ⚠️ NON-CANONICAL — NOT WIRED INTO THE LIVE APP ⚠️
 *
 * Document 04's canonical implementation is `AssessmentForm.tsx` (the
 * Assessment/FBA Record, which receives and reconciles Frame's
 * `FbaOutcomeBundle` — see `CONTRADICTIONS.md` #6). This form — full
 * from-scratch practitioner authoring of the same fields, no Frame
 * handoff — predates that decision and is preserved unmodified as a
 * possible future "Frame unavailable" manual-authoring fallback, per
 * `CONTRADICTIONS.md` #12. Whether Vector needs that fallback mode at
 * all is a separate, not-yet-made decision. `VectorApp` does not mount
 * this component for document "04"; it is exported from `index.ts` only
 * so it stays reachable as a library symbol, not as a second navigable
 * document-04 route.
 */
import {
  CAPABILITIES,
